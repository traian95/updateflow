...
def render_category_block(cursor, titlu: str, furnizor: str, readonly: bool) -> None:
    if readonly:
        return
    sl = _slug(titlu)
    ...
    with st.expander(f"▾ {titlu}", expanded=False):
        ...
        if titlu == "Manere" and man_f == "Enger":
            ...
            if st.button("Adaugă (Enger)", key=f"add_eng_{sl}"):
                if total is None:
        ...
        if is_parchet:
            ...
            if st.button("Calculează și adaugă", key=f"add_parchet_cat_{sl}"):
                try:
        ...
        if st.button("ADĂUGĂ", key=f"add_cat_{sl}"):
            if pret_val <= 0:
...
def render_configurator() -> None:
    ...
    if st.session_state.dev_mode:
        st.info("Mod Dev — fără validare client.")
    ...
    with right:
        ...
        st.write(f"Valoare totală (TVA INCLUS): **{totals['total_fara_disc_lei']:.2f} RON**")
        st.write(f"Discount aplicat (RON): **{totals['discount_ron']:.2f} RON**")
        st.write(f"Valoare cu discount (TVA INCLUS): **{totals['ultima_valoare_lei']:.2f} RON**")
        st.write(f"AVANS (40%): **{totals['avans_40']:.2f} RON**")
        ...
        if not readonly:
            if st.button("SALVEAZĂ OFERTA", type="primary"):
        ...
        if st.button("📥 Generează PDF (descarcă)"):
            if not st.session_state.cos:
